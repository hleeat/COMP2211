...
def reshape_x(x_train_raw, x_val_raw, x_test_raw):
    ...
    return (
        np.reshape(x_train_raw,(x_train_raw.shape[0],28,28,1)),
        np.reshape(x_val_raw,(x_val_raw.shape[0],28,28,1)),
        np.reshape(x_test_raw,(x_test_raw.shape[0],28,28,1)),
...
def compile_model(model, lr):
    ...
    sdg=tf.keras.optimizers.SGD(lr=lr)
...
def predict_images(model, x):
    ...
    pre=model.predict(x)
    return np.array(pre.argmax(axis=1))
