...
def get_X_y(df):
  ...
  X=[preprocessing(x) for x in df['review']]
  ...
  y=np.array([int(t=='positive') for t in df['sentiment']])
  ##### END of TODO
  return X,y
...
def formatEmbDict(filepath):
  ...
  for m in content:
    n=m.split()
    wordDict[n[0]]=np.array(n[1:],dtype=np.float32)
...
def myModel(vocab_size, embedding_matrix, maxlen):
  ...
  model.add(Flatten())
  model.add(Dense(50, activation='relu',kernel_regularizer=regularizers.l2(0.003)))
  model.add(Dropout(0))
  model.add(Dense(1, activation='sigmoid',kernel_regularizer=regularizers.l2(0.003)))
